...
Пример:  
![Схема размещения элементов страницы-шаблона второстепенной страницы][image1]
...
2) Описать ключевые слова в метаинформации страницы, а также в описании страницы
3) Для упрощения проектирования подключить файл стилей outlines.css (опционально)
4) Приступить к написанию сайта:
* Добавить Doctype
...
Пример shadow drop:  
![Drop Shadow Effect For Images \&amp; Videos – Image-shadow][image2]
...
7. Отступы
8. пробелы
9. Стилизация  переформатированного текста
10. Подчеркивание, зачеркивание и  другие
11. Регистр символов
...
3) Добавить на страницу таблицу с заголовками и стилями для чётных и нечётных строк. В качестве типа отображения таблицы рекомендуется в	оспользоваться grid’ом, для достижения лучшей адаптивности. (т.е. необходимо добиться того, чтобы колонки таблицы изменяли свой размер в зависимости от размера окнаха)  
   пример:  
   ![How to display table rows in alternating colors with CSS | Tutorial |  Example | September 2021 | HowtoASP.NET][image3]
...
Примерный ожидаемый результат лабораторной работы на примере проекта «список дел»: [https://i.imgur.com/y5m4LyI.gif](https://i.imgur.com/y5m4LyI.gif)
...
3) Дождаться события загрузки страницы и инициализовать обращение к поставщику данных используя Fetch API ([https://developer.mozilla.org/ru/docs/Web/API/Fetch\_API](https://developer.mozilla.org/ru/docs/Web/API/Fetch_API))
...
1) Выберите одну – две UI библиотеки которые можно интегрировать в ваш проект из предложенного списка: [https://github.com/sorrycc/awesome-javascript](https://github.com/sorrycc/awesome-javascript#sliders)
   ...
   [https://swiperjs.com/demos/130-centered/core.html](https://swiperjs.com/demos/130-centered/core.html)
